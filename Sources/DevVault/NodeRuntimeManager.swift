...
actor NodeRuntimeManager {
    ...
    private func ensureRuntime(for requirement: NodeRuntimeRequirement) async throws -> ResolvedNodeRuntime {
        ...
        guard installResult.exitCode == 0 else {
            throw DevVaultError.commandFailed(installResult.stderr.isEmpty ? installResult.stdout : installResult.stderr)
        }
        ...
        guard nodePath.hasPrefix("/") else {
            throw DevVaultError.commandFailed("Unable to resolve the managed Node runtime for \(requirement.nodeVersionSpec).")
        }

        let versionResult = try await runNVMCommand("nvm version \(shellQuote(installableSpec))")
        guard versionResult.exitCode == 0 else {
            throw DevVaultError.commandFailed(versionResult.stderr.isEmpty ? versionResult.stdout : versionResult.stderr)
        }
    ...
    private func resolveInstallableSpec(for rawSpec: String) async throws -> String {
        ...
        throw DevVaultError.commandFailed("No installable Node.js version matches '\(trimmed)'.")
    }
    ...
    private func bootstrapNVMIfNeeded(allowUpdate: Bool) async throws {
        ...
        guard updateResult.exitCode == 0 else {
            throw DevVaultError.commandFailed(updateResult.stderr.isEmpty ? updateResult.stdout : updateResult.stderr)
        }
    }
